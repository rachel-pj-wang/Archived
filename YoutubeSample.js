...
exports.handler = (event, context) => {
  ...
  try {
    ...
    switch (event.request.type) {
      ...
      case "GetTest":{
        var voteSmart = "http://api.votesmart.org/Votes.getBill?&key=8357d7f39e61304dbdeebd2e1772a40f&billId=23500"

        Https.get(voteSmart, (response) => {
          response.on('data', (data) => {
            var convert = xmlToJson(data)
            var voteJSON = JSON.parse(convert)


                context.succeed(
                  generateResponse(
                    buildSpeechletResponse("it worksss", true),
                    {}
                  )
                )
              })
